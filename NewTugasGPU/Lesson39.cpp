...
BOOL Initialize39(GL_Window* window, Keys* keys)					// Any GL Init Code & User Initialiazation Goes Here
{
	g_window39 = window;
	g_keys39 = keys;
...
void Deinitialize39(void)										// Any User DeInitialization Goes Here
{
	KillFont();
...
void Update39(DWORD milliseconds)								// Perform Motion Updates Here
{
	if (g_keys39->keyDown[VK_ESCAPE] == TRUE)					// Is ESC Being Pressed?
		TerminateApplication(g_window39);						// Terminate The Program

	if (g_keys39->keyDown[VK_F1] == TRUE)						// Is F1 Being Pressed?
		ToggleFullscreen(g_window39);							// Toggle Fullscreen Mode

	if (g_keys39->keyDown[VK_F2] == TRUE)						// Is F2 Being Pressed?
		slowMotionRatio = 1.0f;									// Set slowMotionRatio To 1.0f (Normal Motion)

	if (g_keys39->keyDown[VK_F3] == TRUE)						// Is F3 Being Pressed?
