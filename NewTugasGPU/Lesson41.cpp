...
GL_Window*	g_window41;													// Window Structure
Keys*		g_keys41;														// Keyboard
...
BOOL Initialize41(GL_Window* window, Keys* keys)							// Any GL Init Code & User Initialiazation Goes Here
{
	g_window41 = window;												// Window Values
	g_keys41 = keys;													// Key Values
...
void Deinitialize41(void)												// Any User DeInitialization Goes Here
{
}

void Update41(DWORD milliseconds)										// Perform Motion Updates Here
{
	if (g_keys41->keyDown[VK_ESCAPE])									// Is ESC Being Pressed?
		TerminateApplication(g_window41);								// Terminate The Program

	if (g_keys41->keyDown[VK_F1])										// Is F1 Being Pressed?
		ToggleFullscreen(g_window41);									// Toggle Fullscreen Mode

	if (g_keys41->keyDown[VK_UP] && camz<14.0f)							// Is UP Arrow Being Pressed?
		camz += (float)(milliseconds) / 100.0f;								// Move Object Closer (Move Forwards Through Hallway)

	if (g_keys41->keyDown[VK_DOWN] && camz>-19.0f)						// Is DOWN Arrow Being Pressed?
