...
GL_Window*	g_window47;													// Window Structure
Keys*		g_keys47;														// Keyboard
...
BOOL Initialize47(GL_Window* window, Keys* keys)							// Any GL Init Code & User Initialiazation Goes Here
{
	g_window47 = window;												// Window Values
	g_keys47 = keys;													// Key Values
	...
	cgProgram = cgCreateProgramFromFile(cgContext, CG_SOURCE, "/Data/CG/Wave.cg", cgVertexProfile, "main", 0);
...
void Deinitialize47(void)												// Any User DeInitialization Goes Here
{
	// Destroy Our Cg Context And All Programs Contained Within It
	cgDestroyContext(cgContext);
}

void Update47(float milliseconds)										// Perform Motion Updates Here
{
	//if (g_keys47->keyDown[VK_ESCAPE])									// Is ESC Being Pressed?
		//TerminateApplication(g_window47);								// Terminate The Program

	//if (g_keys47->keyDown[VK_F1])										// Is F1 Being Pressed?
		//ToggleFullscreen(g_window47);									// Toggle Fullscreen Mode

	//if (g_keys47->keyDown[' '] && !sp)
	//{
	//	sp = TRUE;
		//cg_enable = !cg_enable;
	//}
