...
GL_Window*	g_window42;													// Window Structure
Keys*		g_keys42;														// Keyboard
...
BOOL Initialize42(GL_Window* window, Keys* keys)							// Any GL Init Code & User Initialiazation Goes Here
{
	tex_data = new BYTE[width*height * 3];									// Allocate Space For Our Texture

	g_window42 = window;												// Window Values
	g_keys42 = keys;													// Key Values
...
void Deinitialize42(void)												// Any User DeInitialization Goes Here
{
	delete[] tex_data;													// Delete Our Texture Data (Freeing Up Memory)
}

void Update42(float milliseconds)										// Perform Motion Updates Here
{
	int		dir;														// Will Hold Current Direction

	if (g_keys42->keyDown[VK_ESCAPE])									// Is ESC Being Pressed?
		TerminateApplication(g_window42);								// Terminate The Program

	if (g_keys42->keyDown[VK_F1])										// Is F1 Being Pressed?
		ToggleFullscreen(g_window42);									// Toggle Fullscreen Mode

	if (g_keys42->keyDown[' '] && !sp)									// Check To See If Spacebar Is Pressed
	{
		sp = TRUE;														// If So, Set sp To TRUE (Spacebar Pressed)
		Reset();														// If So, Call Reset And Start A New Maze
	}

	if (!g_keys42->keyDown[' '])											// Check To See If Spacebar Has Been Released
	...
	if (done)															// If done Is True Then There Were No Unvisited Rooms
	{
		...
		SetWindowText(g_window42->hWnd, "Lesson 42: Multiple Viewports... 2003 NeHe Productions... Maze Complete!");
		Sleep(5000);
		SetWindowText(g_window42->hWnd, "Lesson 42: Multiple Viewports... 2003 NeHe Productions... Building Maze!");
		Reset();
	}
...
void Draw42(void)														// Our Drawing Routine
{
	RECT	rect;														// Holds Coordinates Of A Rectangle

	GetClientRect(g_window42->hWnd, &rect);								// Get Window Dimensions
