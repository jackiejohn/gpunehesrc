			...
			for (ki = 0; ki<3; ki++)
				if (!o->planes[i].neigh[ki]) {
					for (kj = 0; kj<3; kj++) {
						...
						P1i = ((p1i + p2i) - abs(static_cast<int>(p1i - p2i))) / 2;
						P2i = ((p1i + p2i) + abs(static_cast<int>(p1i - p2i))) / 2;
						P1j = ((p1j + p2j) - abs(static_cast<int>(p1j - p2j))) / 2;
						P2j = ((p1j + p2j) + abs(static_cast<int>(p1j - p2j))) / 2;
