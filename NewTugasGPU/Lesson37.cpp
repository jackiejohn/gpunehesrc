...
// Engine Functions
BOOL Initialize37(GL_Window* window, Keys* keys)					// Any GL Init Code & User Initialiazation Goes Here
{
...
void Deinitialize37(void)										// Any User DeInitialization Goes Here
{
...
void Update37(DWORD milliseconds)								// Perform Motion Updates Here
{
	if (g_keys37->keyDown[VK_ESCAPE] == TRUE)					// Is ESC Being Pressed?
	{
		TerminateApplication(g_window37);						// Terminate The Program
	}

	if (g_keys37->keyDown[VK_F1] == TRUE)						// Is F1 Being Pressed?
	{
		ToggleFullscreen(g_window37);							// Toggle Fullscreen Mode
	}

	if (g_keys37->keyDown[' '] == TRUE)							// Is the Space Bar Being Pressed? ( NEW )
	{
		modelRotate = !modelRotate;								// Toggle Model Rotation On/Off ( NEW )

		g_keys37->keyDown[' '] = FALSE;
	}

	if (g_keys37->keyDown['1'] == TRUE)							// Is The Number 1 Being Pressed? ( NEW )
	{
		outlineDraw = !outlineDraw;								// Toggle Outline Drawing On/Off ( NEW )

		g_keys37->keyDown['1'] = FALSE;
	}

	if (g_keys37->keyDown['2'] == TRUE)							// Is The Number 2 Being Pressed? ( NEW )
	{
		outlineSmooth = !outlineSmooth;							// Toggle Anti-Aliasing On/Off ( NEW )

		g_keys37->keyDown['2'] = FALSE;
	}

	if (g_keys37->keyDown[VK_UP] == TRUE)						// Is The Up Arrow Being Pressed? ( NEW )
	{
		outlineWidth++;											// Increase Line Width ( NEW )

		g_keys37->keyDown[VK_UP] = FALSE;
	}

	if (g_keys37->keyDown[VK_DOWN] == TRUE)						// Is The Down Arrow Being Pressed? ( NEW )
	{
		outlineWidth--;											// Decrease Line Width ( NEW )

		g_keys37->keyDown[VK_DOWN] = FALSE;
	}
