...
BOOL Initialize40(GL_Window* window, Keys* keys)								// Any GL Init Code & User Initialiazation Goes Here
{
	g_window40 = window;
	g_keys40 = keys;
...
void Deinitialize40(void)													// Any User DeInitialization Goes Here
{
...
void Update40(DWORD milliseconds)											// Perform Motion Updates Here
{
	if (g_keys40->keyDown[VK_ESCAPE] == TRUE)								// Is ESC Being Pressed?
		TerminateApplication(g_window40);									// Terminate The Program

	if (g_keys40->keyDown[VK_F1] == TRUE)									// Is F1 Being Pressed?
		ToggleFullscreen(g_window40);										// Toggle Fullscreen Mode

	Vector3D ropeConnectionVel;												// Create A Temporary Vector3D

																			// Keys Are Used To Move The Rope
	if (g_keys40->keyDown[VK_RIGHT] == TRUE)									// Is The Right Arrow Being Pressed?
		ropeConnectionVel.x += 3.0f;										// Add Velocity In +X Direction

	if (g_keys40->keyDown[VK_LEFT] == TRUE)									// Is The Left Arrow Being Pressed?
		ropeConnectionVel.x -= 3.0f;										// Add Velocity In -X Direction

	if (g_keys40->keyDown[VK_UP] == TRUE)									// Is The Up Arrow Being Pressed?
		ropeConnectionVel.z -= 3.0f;										// Add Velocity In +Z Direction

	if (g_keys40->keyDown[VK_DOWN] == TRUE)									// Is The Down Arrow Being Pressed?
		ropeConnectionVel.z += 3.0f;										// Add Velocity In -Z Direction

	if (g_keys40->keyDown[VK_HOME] == TRUE)									// Is The Home Key Pressed?
		ropeConnectionVel.y += 3.0f;										// Add Velocity In +Y Direction

	if (g_keys40->keyDown[VK_END] == TRUE)									// Is The End Key Pressed?
		ropeConnectionVel.y -= 3.0f;										// Add Velocity In -Y Direction
