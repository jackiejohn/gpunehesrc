...
BOOL Initialize48(GL_Window* window, Keys* keys)						// Any GL Init Code & User Initialiazation Goes Here
{
	g_window48 = window;
	g_keys48 = keys;
...
void Deinitialize48(void)											// Any User DeInitialization Goes Here
{
	gluDeleteQuadric(quadratic);
}

void Update48(DWORD milliseconds)									// Perform Motion Updates Here
{
	if (g_keys48->keyDown[VK_ESCAPE] == TRUE)						// Is ESC Being Pressed?
		TerminateApplication(g_window48);							// Terminate The Program

	if (g_keys48->keyDown[VK_F1] == TRUE)							// Is F1 Being Pressed?
		ToggleFullscreen(g_window48);								// Toggle Fullscreen Mode
