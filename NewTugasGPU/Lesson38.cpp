...
void LoadGLTextures()											// Creates Textures From Bitmaps In The Resource File
{
	...
	byte	Texture[] = { IDB_BITMAP3,	IDB_BITMAP3,	IDB_BITMAP3 };
...
BOOL Initialize38(GL_Window* window, Keys* keys)					// Any GL Init Code & User Initialiazation Goes Here
{
	g_window38 = window;
	g_keys38 = keys;
...
void Deinitialize38(void)										// Any User DeInitialization Goes Here
{
}

void Update38(DWORD milliseconds)								// Perform Motion Updates Here
{
	if (g_keys38->keyDown[VK_ESCAPE] == TRUE)					// Is ESC Being Pressed?
	{
		TerminateApplication(g_window38);						// Terminate The Program
	}

	if (g_keys38->keyDown[VK_F1] == TRUE)						// Is F1 Being Pressed?
	{
		ToggleFullscreen(g_window38);							// Toggle Fullscreen Mode
	}
}

void Draw38(void)												// Draw The Scene
{
