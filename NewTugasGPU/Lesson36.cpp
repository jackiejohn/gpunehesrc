...
GL_Window*	g_window36;
Keys*		g_keys36;

// User Defined Variables
float		angle36;												// Used To Rotate The Helix
float		vertexes36[4][3];										// Holds Float Info For 4 Sets Of Vertices
float		normal36[3];											// An Array To Store The Normal Data
GLuint		BlurTexture36;										// An Unsigned Int To Store The Texture Number
...
void ProcessHelix()												// Draws A Helix
{
	...
	glRotatef(angle36 / 2.0f, 1, 0, 0);								// Rotate By angle/2 On The X-Axis
	glRotatef(angle36 / 3.0f, 0, 1, 0);								// Rotate By angle/3 On The Y-Axis
	...
	for (phi = 0; phi <= 360; phi += 20.0)							// 360 Degrees In Steps Of 20
	{
		for (theta = 0; theta <= 360 * twists; theta += 20.0)			// 360 Degrees * Number Of Twists In Steps Of 20
		{
			...
			vertexes36[0][0] = x;									// Set x Value Of First Vertex
			vertexes36[0][1] = y;									// Set y Value Of First Vertex
			vertexes36[0][2] = z;									// Set z Value Of First Vertex
			...
			vertexes36[1][0] = x;									// Set x Value Of Second Vertex
			vertexes36[1][1] = y;									// Set y Value Of Second Vertex
			vertexes36[1][2] = z;									// Set z Value Of Second Vertex
			...
			vertexes36[2][0] = x;									// Set x Value Of Third Vertex
			vertexes36[2][1] = y;									// Set y Value Of Third Vertex
			vertexes36[2][2] = z;									// Set z Value Of Third Vertex
			...
			vertexes36[3][0] = x;									// Set x Value Of Fourth Vertex
			vertexes36[3][1] = y;									// Set y Value Of Fourth Vertex
			vertexes36[3][2] = z;									// Set z Value Of Fourth Vertex

			calcNormal(vertexes36, normal36);						// Calculate The Quad Normal

			glNormal3f(normal36[0], normal36[1], normal36[2]);			// Set The Normal

																	// Render The Quad
			glVertex3f(vertexes36[0][0], vertexes36[0][1], vertexes36[0][2]);
			glVertex3f(vertexes36[1][0], vertexes36[1][1], vertexes36[1][2]);
			glVertex3f(vertexes36[2][0], vertexes36[2][1], vertexes36[2][2]);
			glVertex3f(vertexes36[3][0], vertexes36[3][1], vertexes36[3][2]);
		}
	}
...
void RenderToTexture()											// Renders To A Texture
{
	...
	glBindTexture(GL_TEXTURE_2D, BlurTexture36);					// Bind To The Blur Texture
...
BOOL Initialize36(GL_Window* window, Keys* keys)					// Any GL Init Code & User Initialiazation Goes Here
{
	g_window36 = window;
	g_keys36 = keys;

	// Start Of User Initialization
	angle36 = 0.0f;											// Set Starting Angle To Zero

	BlurTexture36 = EmptyTexture();								// Create Our Empty Texture
...
void Deinitialize36(void)										// Any User DeInitialization Goes Here
{
	glDeleteTextures(1, &BlurTexture36);							// Delete The Blur Texture
}

void Update36(DWORD milliseconds)								// Perform Motion Updates Here
{
	if (g_keys36->keyDown[VK_ESCAPE] == TRUE)					// Is ESC Being Pressed?
	{
		TerminateApplication(g_window36);						// Terminate The Program
	}

	if (g_keys36->keyDown[VK_F1] == TRUE)						// Is F1 Being Pressed?
	{
		ToggleFullscreen(g_window36);							// Toggle Fullscreen Mode
	}

	angle36 += (float)(milliseconds) / 5.0f;						// Update angle Based On The Clock
}

void Draw36(void)												// Draw The Scene
{
	glClearColor(0.0f, 0.0f, 0.0f, 0.5);						// Set The Clear Color To Black
